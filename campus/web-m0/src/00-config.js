...
(function (root) {
  ...
  C.Config = {
    ...
    loudness: {
      ...
      /* 放下东西：基础 8，再按重量加最多 12（`8 + 重量×3`）。
         **扔一口 2.2kg 的铁锅（14）和放下一张纸（8）不该一样响。** */
      dropItem: 8,
      meleeHit: 55,
      playerHurt: 60,
      broadcast: 150,
    ...
    power: {
      ...
      /* 电器自带的那根电源线有多长。**这是「灶台放哪」这个决定的全部约束** ——
         1.8 米意味着设备必须放在插座边上那张桌子，而不是房间正中。
         想放远一点就得去找真的电线（15.4），那是另一笔重量和格子的开销。 */
      cordMetres: 1.8,
